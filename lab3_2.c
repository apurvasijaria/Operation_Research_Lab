...
void Data() 
{
	...
	printf(" Whether you want to maximize(Y/N) ? "); 
	scanf("%c", &R);
	printf("\n Number of variables in maximizing function ? ");
	scanf("%d", &NV);
	...
	if (R == 'Y' || R=='y')
		...
		R1 = -1.0;
	printf("\n Enter the coefficients of maxmizing function:\n");
	for (J = 1; J<=NV; J++) 
	{
		printf(" #%d ? ", J); 
		scanf("%lf", &R2);
